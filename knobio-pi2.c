...
int main()
{
	...
	float x;
	long inter;
          sprintf (vol_buffer, "%lu" , ((p+10000)/10380.0)*255); //this math is to reverse the math done the buffer value
...
*/
          ...
          //printf("Buffer after conversion: %ld\n",atol(buf));
		   //x = ((atol(buf)/255.0)*(atol(buf)/255.0)*(atol(buf)/255.0)*(atol(buf)/255.0));
		   x = atol(buf)/255.0;
			volume = (x*10380)-10000;
